...
export default function SonyBrandPage() {
  ...
  useEffect(() => {
    fetch("/api/brands")
      .then((res) => res.json())
      .then((data) => {
        const sony = data.find((b: any) => b.name.toLowerCase() === "sony");
        setModels(sony ? sony.models.map((m: string) => ({ name: m })) : []);
        setLoading(false);
      });
  }, []);
      ...
      <section className="bg-gradient-to-br from-blue-50 to-white py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl text-gray-600 mb-8">
            Popüler Sony TV modelleri için ekran değişimi ve tamir hizmetleri. Modelinizi seçerek detaylı bilgi ve fiyat teklifi alabilirsiniz.
          </p>
        </div>
      </section>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {models.map((model) => (
            <Link
              key={model.name}
              href={`/brands/sony/${model.name.toLowerCase()}`}
